...
func (t *gcsFuseCSIMountTestSuite) DefineTests(driver storageframework.TestDriver, pattern storageframework.TestPattern) {
	...
	testGcsfuseProfilesFlagFilters := func(expectProfile bool, configPrefix ...string) {
		init(configPrefix...)
		defer cleanup()
		...
		tPod.VerifyProfileFlagsArePassed(f.Namespace.Name, "aiml-training", expectProfile)
	...
	ginkgo.It("should pass profile as a user-specified mountOption to gcsfuse when profile is specified and 'enable-gcsfuse-profiles-internal' flag is true", func() {
		testGcsfuseProfilesFlagFilters(true, specs.PassProfilesToSidecarPrefix)
	})
	...
	ginkgo.It("should not pass profile as a user-specified mountOption to gcsfuse when profile is specified and 'enable-gcsfuse-profiles-internal' flag is false", func() {
		testGcsfuseProfilesFlagFilters(false, specs.PassProfilesToSidecarPrefix)
	})
